...
def pickscore_sd3():
    ...
    # sd3.5 medium
    config.pretrained.model = "/data1/xiazheng/flow_grpo/models/stable-diffusion-3.5-medium"
    config.sample.num_steps = 10
    config.sample.eval_num_steps = 40
    config.sample.guidance_scale = 4.5

    config.resolution = 512
    config.sample.train_batch_size = 6 #memo 单张显卡在一次训练迭代（Step）中并行处理的图片数量
    config.sample.num_image_per_prompt = 6 #memo 每个提示词生成多少图片，每组的图片数量;单卡Batch Size乘以显卡数量必须是num_image_per_prompt的整数倍
    config.sample.num_batches_per_epoch = int(48/(gpu_number*config.sample.train_batch_size/config.sample.num_image_per_prompt))
    assert config.sample.num_batches_per_epoch % 2 == 0, "Please set config.sample.num_batches_per_epoch to an even number! This ensures that config.train.gradient_accumulation_steps = config.sample.num_batches_per_epoch / 2, so that gradients are updated twice per epoch."
